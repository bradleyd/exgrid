defmodule ExGrid.BlocksTest do
  ...
  test "blocks filters via email address" do
    { :ok, creds } = ExGrid.credentials(%{api_key: System.get_env("API_KEY"), api_user: System.get_env("API_USER")})
    {200, body} = ExGrid.Blocks.get(creds, %{email: "thisemailshouldnotexist@nowhere.com"})
  ...
  test "blocks filters via limit parameter" do
    { :ok, creds } = ExGrid.credentials(%{api_key: System.get_env("API_KEY"), api_user: System.get_env("API_USER")})
    {200, body} = ExGrid.Blocks.get(creds, %{limit: 1})
    assert Enum.count(body) <= 1
  end

  test "blocks filters via multiple parameters" do
    { :ok, creds } = ExGrid.credentials(%{api_key: System.get_env("API_KEY"), api_user: System.get_env("API_USER")})
    {200, body} = ExGrid.Blocks.get(creds, %{limit: 1, date: 1})
    assert Enum.count(body) <= 1
    #assert HashDict.has_key?(Enum.at(body,0), "created")
  end
  ...
  test "blocks filter via type" do
    { :ok, creds } = ExGrid.credentials(%{api_key: System.get_env("API_KEY"), api_user: System.get_env("API_USER")})
    {200, body} = ExGrid.Blocks.get(creds, %{type: "hard"})
    assert (is_list(body) || is_map(body))
    #assert Enum.count(body) <= 1
  ...
  test "gets blocks by start_date" do
    { :ok, creds } = ExGrid.credentials(%{api_key: System.get_env("API_KEY"), api_user: System.get_env("API_USER")})
    {200, body} = ExGrid.Blocks.get(creds, %{start_date: "2013-09-10"})
    assert (is_list(body) || is_map(body))
    #assert HashDict.has_key?(Enum.at(body,0), "status")
  end
  ...
  test "gets blocks by start_date and end_date" do
    { :ok, creds } = ExGrid.credentials(%{api_key: System.get_env("API_KEY"), api_user: System.get_env("API_USER")})
    {200, body} = ExGrid.Blocks.get(creds, %{start_date: "2013-09-10", end_date: "2014-07-20"})
    assert (is_list(body) || is_map(body))
    #assert HashDict.has_key?(Enum.at(body,0), "status")
  end
  ...
  test "remove block" do
    { :ok, creds } = ExGrid.credentials(%{api_key: System.get_env("API_KEY"), api_user: System.get_env("API_USER")})
    {200, body} = ExGrid.Blocks.remove(creds, %{email: "foo@bar.com"})
